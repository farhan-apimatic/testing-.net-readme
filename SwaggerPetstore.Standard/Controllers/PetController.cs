...
namespace SwaggerPetstore.Standard.Controllers
{
    public partial class PetController: BaseController
    {
        ...
        public List<Models.Pet> FindPetsByStatus(List<Models.Status2Enum> status)
        {
        ...
        /// <param name="status">Required parameter: Status values that need to be considered for filter</param>
        /// <return>Returns the List<Models.Pet> response from the API call</return>
        public async Task<List<Models.Pet>> FindPetsByStatusAsync(List<Models.Status2Enum> status)
        {
            //the base uri for api requests
            string _baseUri = Configuration.GetBaseURI();
            ...
            {
                { "status", Models.Status2EnumHelper.ToValue(status) }
